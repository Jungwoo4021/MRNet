...
from ..dataset import SV_Trial

def SV_enrollment(framework, loader, TTA=False, run_on_ddp=False):
...
def test_SV_EER(trials, single_embedding=None, multi_embedding=None, get_score=False):
    ...
    for i in range(len(trials) // 1000):
        ...
        if single_embedding is not None:
            cos_sims_single += _calculate_cos_sim(sub_trials, single_embedding)
        
        if multi_embedding is not None:
            cos_sims_multi += _calculate_cos_sim(sub_trials, multi_embedding, TTA=True)

    if cos_sims_multi is None:
        cos_sims = cos_sims_single
    elif single_embedding is None:
        cos_sims = cos_sims_multi        
    else:
        cos_sims = [(cos_sims_single[i] + cos_sims_multi[i]) / 2 for i in range(len(cos_sims_single))]
...
def adaptive_s_norm(path, dataset, framework, trials, eval_embeddings, num_spk=600, path_cohort_set=None, cross_select=False, top_n=300, return_scores=False):
    # sample cohort set
    ...
    # asnorm
    output_scores = asnorm_score(score_enroll_test, score_enroll_cohort, score_test_cohort, cross_select, top_n)
...
def enrollment_cohort_embedding(framework, path_cohort_set):
    ...
    for line in f.readlines():
        ...
        speaker = strI[0]
        embeddings[speaker] = []
        items += [(speaker, file) for file in strI[1:]]
    ...
    with torch.set_grad_enabled(False):
        for key, wav in tqdm(loader, ncols=90, desc='cohort enrollment'):
...
def asnorm_score(f_enroll_test, f_enroll_cohort, f_test_cohort, cross_select=False, top_n=300, output_statistics=False):
    ...
    for _, row in tqdm(input_score.iterrows(), ncols=90, desc='asnorm'):
        ...
        if output_statistics:
            output_score.append([enroll_key, test_key, normed_score, enroll_mean[enroll_key], \
                enroll_std[enroll_key], test_mean[test_key], test_std[test_key]])
        else:
            output_score.append([enroll_key, test_key, normed_score])

    return output_score

def qmf(sv_framework, dataset, path_train_set=None, len_long=6, len_max=30, num_trials=30000):
    '''
    # read train_set.txt
    if path_train_set is None:
        samples = sample_QMF_train_set(path_train_set, dataset, len_long, len_max, num_trials)
    utts = []
    count = 0
    for item in samples:
        utts.append((count, item.key1))
        count += 1
        utts.append(count, item.key2)
        count += 1
    
    # enrollment
    embeddings = {}
    loader = DataLoader(WavLoader(utts), num_workers=10, batch_size=1, pin_memory=True)
    ...
    cos_scores = []
    for item in samples:
        wav, _ = sf.read(item.key1)
        wav = torch.
        
    embeddings = SV_enrollment(sv_framework, samples, run_on_ddp)
    '''
    pass
...
def sample_QMF_train_set(path, dataset, len_long=6, len_max=30, num_trials=30000):
    ...
    while len(samples) < num_trials // 6 * 2:
        spk = random.choice(speakers)
        if len(short_utts[spk]) < 1 or len(long_utts[spk]):
            continue
        utt1 = short_utts[spk].pop(random.randint(0, len(short_utts[spk]) - 1))
