...
class StudentWavLMPlus_DistHubt(nn.Module):
    def __init__(self, num_hidden_layer, hidden_size, return_all_hiddens=False, init_teacher_param=None, ft_adapter_hidden_size=None):
        ...
        self.wavlm = CustomWavLMModel(config=config, ft_adapter_hidden_size=ft_adapter_hidden_size)
...
class StudentWavLMPlus_FitHubt(nn.Module):
    def __init__(self, num_hidden_layer, hidden_size_s, hidden_size_t, sequence_length, return_all_hiddens=False, ft_adapter_hidden_size=None):
        ...
        self.wavlm = CustomWavLMModel(config, use_time_reduction=True, ft_adapter_hidden_size=ft_adapter_hidden_size)
...
class CustomWavLMModel(WavLMModel):
    def __init__(self, config, use_time_reduction=False, ft_adapter_hidden_size=None):
        ...
        if config.do_stable_layer_norm:
            self.encoder = CustomWavLMEncoderStableLayerNorm(config, ft_adapter_hidden_size)
        else:
            self.encoder = CustomWavLMEncoder(config, ft_adapter_hidden_size)
...
class CustomWavLMEncoder(nn.Module):
    def __init__(self, config, ft_adapter_hidden_size=None):
        ...
        self.layers = nn.ModuleList(
            [CustomWavLMEncoderLayer(config, has_relative_position_bias=(i == 0), ft_adapter_hidden_size=ft_adapter_hidden_size) for i in range(config.num_hidden_layers)]
        )
...
class CustomWavLMEncoderLayer(nn.Module):
    def __init__(self, config, has_relative_position_bias: bool = True, ft_adapter_hidden_size=None):
        ...
        if self.use_ft_adapter:
            self.ft_projector = TargetTaskFeatureProjector(config.hidden_size, ft_adapter_hidden_size)
            self.ft_attention = TargetTaskFeatureAttention(config.hidden_size)
    ...
    def forward(self, hidden_states, attention_mask=None, position_bias=None, output_attentions=False, index=0, idx_without_adapter=None):
        ...
        if self.use_ft_adapter:
            if idx_without_adapter is None:
                h = self.ft_projector(hidden_states)    
                hidden_states = hidden_states + self.feed_forward(hidden_states) + h
                hidden_states = self.ft_attention(hidden_states)
            else:
                ...
                kd_ff_hidden_states, ft_ff_hidden_states = ff_hidden_states[:idx_without_adapter, :, :], hidden_states[idx_without_adapter:, :, :]

                # KD branch
                kd_hidden_states = kd_hidden_states + kd_ff_hidden_states
                
                # FT branch
                h = self.ft_projector(ft_hidden_states)
                ft_hidden_states = ft_hidden_states + ft_ff_hidden_states + h
                ft_hidden_states = self.ft_attention(ft_hidden_states)
...
class TargetTaskFeatureProjector(nn.Module):
...
class TargetTaskFeatureAttention(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        
        self.w = nn.Linear(in_channels * 2, 1)
        self.softmax = nn.Softmax(dim=-1)
    
    def forward(self, x):
        batch, time, hidden = x.size()
        x1 = x.repeat(1, 1, time).view(batch, time * time, hidden)
        x2 = x.repeat(1, time, 1).view(batch, time * time, hidden)
        w = torch.cat((x1, x2), dim=-1)
        w = self.w(w).view(batch, time, time)
        w = self.softmax(w).view(batch, time, time, 1)
        
        x = x.repeat(1, time, 1).view(batch, time, time, hidden)
        x = x * w
        x = x.sum(dim=2)
